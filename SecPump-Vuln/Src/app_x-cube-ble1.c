...
uint8_t bnrg_expansion_board =
    IDB04A1; /* at startup, suppose the X-NUCLEO-IDB04A1 is used */
...
void print_csv_time(void) {
  ...
  PRINT_CSV("%02d:%02d:%02d.%03d", ms / (60 * 60 * 1000) % 24,
            ms / (60 * 1000) % 60, (ms / 1000) % 60, ms % 1000);
}
#endif
...
void MX_BlueNRG_MS_Init(void) {
  ...
  ret = Add_Pump_Service();

  if (ret == BLE_STATUS_SUCCESS)
    printf("[*] SecPump service added successfully.\r\n");
  else
